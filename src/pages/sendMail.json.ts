...
export async function POST({ params, request }) {
	...
	try {
		const logMessage = `${new Date().toString()}: Dados do lead - ${JSON.stringify(dataValue)}\n`
		...
		if (refererDomain.toLowerCase().indexOf('digitalspark') > -1) target = 'digitalspark'
		else if (refererDomain.toLowerCase().indexOf('rushmedia') > -1) target = 'rushmedia'
